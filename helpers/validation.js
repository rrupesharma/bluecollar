...
Validator.registerAsync(
  "checkToken",
  async function (value, attribute, req, passes) {
    let query = `select * from admin_tbl where token='${value}'`;
    let result = await pool.executeQuery(query, []);
    if (result.rows.length > 0) {
      let jwtDataDecord = await jwtToken.verify(result.rows[0]["token"]);
      if (jwtDataDecord == false) {
        passes(false, "Invalid token");
      } else {
        if (
          jwtDataDecord.action == "verification" ||
          jwtDataDecord.action == "reset"
        ) {
          passes();
        } else {
          passes(false, "Invalid token");
