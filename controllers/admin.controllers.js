...
const setForgotPassword = async (req, res) => {
  try {
    ...
    let password = req.body.password;
    let query = `update admin_tbl set password='${md5(
      password
    )}' where token='${token}'`;
    // console.log(query);
...
const sendForgotPasswordLink = async (req, res) => {
  try {
    ...
    let token = await jwtToken.generate(
...
const checkVerificationLink = async (req, res) => {
  try {
    ...
    if (!token_data) {
      ...
      //               VALUES (${result[0].accno},'${result[0].email}', '${
      //     result[0].name
      //   }', '', '${result[0].phone}', '${username}', '${md5(password)}', '${
      //     token_data.role
      //   }', now(), true) returning id;`;
      //   await pool.executeQuery(query, []);
      // } else {
      query = `select  * from admin_tbl where token='${token}'`;
      let result = await pool.executeQueryWithMsg(query, [], "Invalid token.");
      query = `update admin_tbl set password='${md5(
        password
      )}',username='${username}',token='',is_active = true where token='${token}'`;
      await pool.executeQuery(query, []);
      // }
      return returnStatus(res, {}, 200, "success");
    }
...
const create = async (req, res) => {
  try {
    ...
    let role = "admin";
    let token = await jwtToken.generate(
      { email: data.email, action: "verification", role: role },
      48
    );
    let query = `INSERT INTO public.admin_tbl(
            email, username, first_name, last_name, phone, password, token, role, created_by, creation_dt)
            VALUES ('${data.email}', '${data.username}', '${
      data.first_name
    }', '${data.last_name}', '${data.phone || ""}', '${md5(
      data.password
    )}', '${token}', '${role}', ${
      req.user.admin_id
    }, now()) returning admin_id`;
...
const getById = async (req, res) => {
  try {
    ...
    let query = `select * from public.admin_tbl where admin_id = ${id} and is_delete = false`;
    let result = await pool.executeQueryWithMsg(
      query,
      [],
  ...
  try {
    ...
    let key =
      req.body.key != undefined && req.body.key != ""
        ? `and (lower(first_name) like lower('%${req.body.key}%') or lower(last_name) like lower('%${req.body.key}%') or lower(email) like lower('%${req.body.key}%')) `
        : "";
    ...
    query = `select count(admin_id) as cnt from public.admin_tbl where is_delete = false ${key}`;
...
const updateById = async (req, res) => {
  try {
    ...
    let query = `UPDATE public.admin_tbl
        SET email='${data.email}', username='${data.username}', first_name='${
      data.first_name
    }', last_name='${data.last_name}', phone='${
      data.phone || ""
    }', updated_by=${req.user.admin_id}, updated_dt=NOW()
...
const deleteById = async (req, res) => {
  try {
    ...
    await validate(req.params, rules, []);
    let id = req.params.id;
    let query = `update public.admin_tbl set is_delete = true where admin_id = ${id}`;
